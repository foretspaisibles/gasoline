(* Gasoline_Plain_Definition -- Basic types for C-stylish applications
...
struct
  ...
  let get k v =
    ...
    | Some(x) -> x
    | None -> failwith "Gasoline_Plain_Definition.Value.get"
  ...
  type a. a kind -> string -> a =
    ...
    | _ ->
      ...
      with Scan_failure(_) -> failwith "Gasoline_Plain_Definition.Value.of_string_kind"
  ...
  let of_string s =
    ...
    let rec loop l =
      match l with
      | [] -> failwith "Gasoline_Plain_Definition.Value.of_string"
