...
struct
  ...
  let put_escaped_string ctxt v =
    String.iter (put_escaped_char ctxt)
  ...
  let add_formatted scratch l ax f v =
    let ctxt = {
      c_locale = l;
      c_format = f;
      c_accumulator = ax;
    ...
    | _ -> invalid_arg "output_formatted"
...
struct
  ...
  let wrap f x =
    try f x
    with Failure _ -> Configuration.parse_error Lexing.dummy_pos x
  ...
  let char_of_string s =
    if String.length s = 1 then
      s.[0]
    else Configuration.parse_error Lexing.dummy_pos s
