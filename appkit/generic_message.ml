...
struct
  ...
  struct
    ...
    type classification = P.Classification.t
    type connection_token = P.Database.connection_token
	...
	with Not_found -> error (Variable_not_found id)
      in
      let concrete =
	try P.Database.find s.database id
	...
	with Lexer.Error _ -> error (Invalid_message concrete)
      in
      begin
	P.Classification.add_pre s.locale s.accumulator cls id;
	List.iter (add_lexeme s.scratch s.locale s.accumulator lookup) lexemes;
	P.Classification.add_post s.locale s.accumulator cls id;
      end
  end
end
...
struct
  ...
  struct
    ...
    type message = {
      m_classification : P.Classification.t;
      m_id : string;
    ...
    let really_send h m =
      let cls = m.m_classification in
      let id = m.m_id in
      let binding = m.m_binding in
      let locale = h.scribe.Scribe.locale in
      begin
	Scribe.add h.scribe cls id binding;
	P.Classification.send_pre locale h.out_channel cls id;
	Scribe.output h.out_channel h.scribe;
	P.Classification.send_post locale h.out_channel cls id;
...
struct
  ...
  sig
    ...
    type control = {
      ...
      mutable delay : bool;
      mutable drop : bool;
      mutable cap: classification option;
      mutable floor: classification option;
    }
    val get_control : t -> control
  ...
  struct
	...
	(f x; x)
      ...
      let finalise x =
	let finally x =
	  P.Database.close x.database
	in
	Gc.finalise finally x

      let make c l =
	let d = P.Database.opendb c in
	...
	begin
	  P.Classification.send_pre locale out_channel cls id;
	  Scribe.output out_channel c.scribe;
	  P.Classification.send_post locale out_channel cls id;
	end
      in
      begin
	...
	let is_greater cls1 cls2 =
	  P.Classification.compare cls1 cls2 >= 0
	in
